...
const HolidayWidget = () => {
  const { user } = useAuth();
  const [upcomingHolidays, setUpcomingHolidays] = useState<HolidayWidgetItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [nearestHoliday, setNearestHoliday] = useState<HolidayWidgetItem | null>(null);
  ...
  useEffect(() => {
    const fetchHolidays = async () => {
      ...
      try {
        ...
        const holidays = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as HolidayWidgetItem));
  ...
  useEffect(() => {
    if (nearestHoliday) {
      const updateCountdown = () => {
        const now = moment();
        const holidayDate = nearestHoliday.sortDate?.clone().startOf('day') ?? moment(); // Use the sorted date object we created
