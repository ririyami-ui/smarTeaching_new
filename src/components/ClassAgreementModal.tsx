...
interface ClassAgreementModalProps {
  isOpen: boolean;
  onClose: () => void;
  classId: string;
  rombel: string;
...
export default function ClassAgreementModal({ isOpen, onClose, classId, rombel, level }: ClassAgreementModalProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchAgreement = async () => {
      if (!isOpen || !classId || !user) return;
      setLoading(true);
      try {
        // Fetch Agreement
        const docRef = doc(db, 'class_agreements', `${user.uid}_${classId}`);
        ...
        let fetchedStudents = studentsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Student));
        ...
        if (fetchedStudents.length === 0 && rombel) {
          ...
          fetchedStudents = legacySnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Student));
        }

        // Sort by attendance number (absen)
        fetchedStudents.sort((a, b) => (Number(a.absen) || 0) - (Number(b.absen) || 0));
          ...
          <div className="ml-auto">
            <StyledButton
              ...
              onClick={() => {
                ...
                (generateClassAgreementPDF as (params: {
                  classData: { level: string; rombel: string };
                  agreementData: { knowledgeWeight: number; practiceWeight: number; academicWeight: number; attitudeWeight: number; agreements: string };
                  ...
                  userProfile,
                  teacherName: String(userProfile?.name || 'Guru'),
                  students: students
                });
              }}
