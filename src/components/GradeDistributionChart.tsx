...
const GradeDistributionChart: React.FC = () => {
    ...
    const [stats, setStats] = useState<Stats>({ avg: 0, median: 0, total: 0 });
    ...
    const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload }) => {
        if (active && payload && payload.length) {
            const chartPayload = payload[0].payload as Record<string, unknown>;
            return (
                <div className="bg-white/80 dark:bg-black/80 backdrop-blur-xl p-4 rounded-2xl shadow-2xl border border-white/20 dark:border-white/10 ring-1 ring-black/5">
                    <p className="font-black text-gray-900 dark:text-white mb-2 tracking-tight">{(chartPayload.name as string || '').replace('\n', ' ')}</p>
                    <div className="space-y-1.5 text-xs">
                        <div className="flex items-center justify-between gap-4">
                            <span className="text-gray-500 dark:text-gray-400 font-bold uppercase tracking-widest">Jumlah</span>
                            <span className="text-gray-900 dark:text-white font-black">{String(chartPayload.Jumlah ?? '')}{' '}<span className="text-[10px] font-bold text-gray-400">Siswa</span></span>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                            <span className="text-gray-500 dark:text-gray-400 font-bold uppercase tracking-widest">Persentase</span>
                            <span className="text-gray-900 dark:text-white font-black">{String(chartPayload.percentage ?? '')}%</span>
                        </div>
                    </div>
                </div>
            );
