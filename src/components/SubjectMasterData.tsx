...
export default function SubjectMasterData() {
  ...
  const bskapSubjects = bskapData.subjects as Record<string, Record<string, Record<string, unknown>>>;
  ...
  const [selectedSchoolLevel, setSelectedSchoolLevel] = useState<string>((userProfile?.schoolLevel as string) || 'SD');
  ...
  useEffect(() => {
    const getSubjects = async () => {
      if (user) {
        ...
        setSubjects(data.docs.map((doc) => ({ ...doc.data(), id: doc.id } as SubjectItem)));
      }
    };
    ...
    return () => unsubscribe();
  }, [subjectsCollectionRef, user]);

  // Sync with profile level changes
  useEffect(() => {
    if (userProfile?.schoolLevel && !editingSubjectId) {
      setSelectedSchoolLevel(userProfile.schoolLevel as string);
    }
  ...
  const startEditing = (subject: SubjectItem) => {
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                ...
                {(() => {
                  ...
                  const levelData = bskapSubjects[level];
                  const subjectsList = levelData
                    ? [...new Set((Object.values(levelData) as Record<string, unknown>[]).flatMap((grade) => Object.keys(grade)))].sort()
                    : [];
                ...
                {editingSubjectId && editedSubjectName &&
                  (!bskapSubjects[selectedSchoolLevel === 'SMK' ? 'SMA' : selectedSchoolLevel] ||
                    !(Object.values(bskapSubjects[selectedSchoolLevel === 'SMK' ? 'SMA' : selectedSchoolLevel] || {}) as Record<string, string[]>[])
