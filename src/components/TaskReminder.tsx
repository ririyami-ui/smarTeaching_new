...
const TaskReminder = ({ user, activeSemester, academicYear }: TaskReminderProps) => {
  ...
  useEffect(() => {
    const checkTasks = async () => {
      ...
      try {
        ...
        const allPending: TaskData[] = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as TaskData));
        ...
        <div className="flex gap-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {[...taskStats.overdue, ...taskStats.upcoming].slice(0, 3).map((task) => {
