...
const ProtaView = ({ grade, subject, semester, year, activeTab, userProfile, signingLocation, sharedEfektifData, subjects }) => {
    ...
    const deleteRow = (id) => setProtaData(protaData.filter(d => d.id !== id));
    ...
    const handleSyncFromATP = async () => {
        ...
        try {
            ...
            if (atpSnap.exists() && atpSnap.data().atpItems) {
                const mappedProta = atpSnap.data().atpItems.map((item, index) => ({
                    id: index + 1,
                    elemen: item.elemen || '',
                    materi: item.materi || '',
                    kd: item.tp || '',
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const data = [
                ...
                ...protaData.map((r, i) => [i + 1, r.elemen, r.materi, r.kd, parseInt(r.jp || 0), r.profilLulusan || '-']),
                ['TOTAL', '', '', '', totalJP, ''],
                [],
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {protaData.map((row, index) => (
                            <tr key={row.id} className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                ...
                                {['elemen', 'materi', 'kd'].map(field => (
                                    <td key={field} className="px-3 py-2 border-r dark:border-gray-700">
                                        <textarea rows="2" value={row[field] || ''} placeholder={isLocked ? '' : `${field}...`} readOnly={isLocked} onChange={(e) => updateRow(row.id, field, e.target.value)} className={`w-full p-2 text-sm border-none bg-transparent focus:ring-1 focus:ring-blue-500 rounded resize-none dark:text-white outline-none ${isLocked ? 'cursor-default' : ''}`} />
                                    </td>
                                ))}
