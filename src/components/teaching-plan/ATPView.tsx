...
import {
    Save, FileText, Download, Workflow, Plus, Trash, Zap, Loader2
} from 'lucide-react';
...
const ATPView: React.FC<ATPViewProps> = ({ grade, subject, semester, year, userProfile, signingLocation, sharedEfektifData }) => {
    const { user } = useAuth();
    const { geminiModel } = useSettings();
    const [atpItems, setAtpItems] = useState<ATPItem[]>([]);
    ...
    useEffect(() => {
        if (sharedEfektifData) {
            if (sharedEfektifData.totalEffectiveHours && sharedEfektifData.totalEffectiveHours > 0) setManualTotalJP(sharedEfektifData.totalEffectiveHours);
            if (sharedEfektifData.jpPerWeek && sharedEfektifData.jpPerWeek > 0) setManualJpPerWeek(sharedEfektifData.jpPerWeek);
            if (sharedEfektifData.totalEffectiveWeeks && sharedEfektifData.totalEffectiveWeeks > 0) setManualTotalWeeks(sharedEfektifData.totalEffectiveWeeks);
        }
    }, [sharedEfektifData]);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            if (sharedEfektifData && sharedEfektifData.totalEffectiveHours && sharedEfektifData.totalEffectiveHours > 0) {
                ...
                setManualJpPerWeek(sharedEfektifData.jpPerWeek || 0);
                setManualTotalWeeks(sharedEfektifData.totalEffectiveWeeks || 0);
            }

            setLoading(true);
            try {
                const id = `${user.uid}_${subject}_${grade}_${year.replace('/', '-')}_${semester}_ATP`;
                setDocId(id);
                ...
                if (!sharedEfektifData || !sharedEfektifData.totalEffectiveHours) {
                    const idProgram = `${user.uid}_${subject}_${grade}_${year.replace('/', '-')}_${semester}`;
    ...
    }, [grade, subject, semester, year, sharedEfektifData, user]);
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            toast.error("Gagal membuat ATP: " + (error as { message?: string }).message);
        } finally {
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            const totalJP = atpItems.reduce((acc, cur) => acc + parseInt(String(cur.jp || 0)), 0);
    ...
    const updateItem = (index: number, field: keyof ATPItem, value: string | number) => {
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div className="p-3 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex justify-between items-center">
                    ...
                    <div className="text-xs font-bold text-gray-500">
                        Total JP: {atpItems.reduce((acc, cur) => acc + parseInt(String(cur.jp || 0)), 0)}
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-wrap justify-end gap-2 pt-4 border-t dark:border-gray-700">
                <button onClick={handleExportPDFATP} disabled={atpItems.length === 0 || isExporting !== ''} className="flex items-center gap-2 px-3 py-2 bg-red-100 text-red-700 rounded hover:bg-red-200 transition text-sm font-semibold disabled:opacity-50">
                    {isExporting === 'pdf' ? <Loader2 className="animate-spin" size={16} /> : <Download size={16} />} PDF
                </button>
