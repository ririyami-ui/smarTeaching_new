...
import {
    Calendar, Save, RefreshCw, Copy, FileSpreadsheet, FileText, Printer
} from 'lucide-react';
...
const PekanEfektifView = ({
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const totalHours = totalEffectiveWeeks * parseInt(jpPerWeek || 0);
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const tableData = months.map(m => [
                m.name, parseInt(m.totalWeeks || 0), parseInt(m.nonEffectiveWeeks || 0),
                (parseInt(m.totalWeeks || 0) - parseInt(m.nonEffectiveWeeks || 0)),
                m.keterangan
            ]);
            const summaryRow = ['TOTAL', months.reduce((a, b) => a + parseInt(b.totalWeeks || 0), 0), months.reduce((a, b) => a + parseInt(b.nonEffectiveWeeks || 0), 0), totalEffectiveWeeks, `Total: ${totalEffectiveWeeks * jpPerWeek} JP`];
    ...
    const handleExportPDF = async () => {
        try {
            ...
            doc.setFont('helvetica', 'normal').text(`${jpPerWeek} JP/Minggu × ${totalEffectiveWeeks} Pekan = ${totalEffectiveWeeks * jpPerWeek} JP`, margins.left + 5, y + 11);
            y += 20;

            autoTable(doc, {
                startY: y,
                head: [['Bulan', 'Jml Pekan', 'Tdk Efektif', 'Efektif', 'Keterangan']],
                body: [...months.map(m => [m.name, m.totalWeeks, m.nonEffectiveWeeks, (parseInt(m.totalWeeks || 0) - parseInt(m.nonEffectiveWeeks || 0)), m.keterangan || '']), ['TOTAL', months.reduce((a, b) => a + parseInt(b.totalWeeks || 0), 0), months.reduce((a, b) => a + parseInt(b.nonEffectiveWeeks || 0), 0), totalEffectiveWeeks, `${totalEffectiveWeeks * jpPerWeek} JP`]],
        ...
        } catch (e) { toast.error("Gagal ekspor PDF."); }
    };
    ...
    const handleExportWord = async () => {
        ...
        const totalHours = totalEffectiveWeeks * jpPerWeek;
        const today = new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
        const signatureHtml = `
    ...
    const totalEffectiveHours = totalEffectiveWeeks * jpPerWeek;
                    ...
                    <tbody>
                        {months.map((month, index) => (
                            <tr key={month.name} className={`${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700/50'} border-b dark:border-gray-700 hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors`}>
                                ...
                                <td className="px-6 py-4 text-center font-bold text-blue-600 border-r">{(parseInt(month.totalWeeks || 0) - parseInt(month.nonEffectiveWeeks || 0))}</td>
                    ...
                    <tfoot className="bg-gray-100 dark:bg-gray-700 font-bold text-gray-900 dark:text-white">
                        <tr>
                            ...
                            <td className="px-4 py-4 border-r text-center">
                                {months.reduce((acc, m) => acc + parseInt(m.totalWeeks || 0), 0)}
                            </td>
                            <td className="px-4 py-4 border-r text-center">
                                {months.reduce((acc, m) => acc + parseInt(m.nonEffectiveWeeks || 0), 0)}
                            </td>
                            ...
                            <td className="px-6 py-4 text-green-700 dark:text-green-400">
                                Total Jam Efektif: {totalEffectiveWeeks * jpPerWeek} JP
                            </td>
                        </tr>
                    </tfoot>
                </table>
                ...
                <Modal onClose={() => setConfirmModal(prev => ({ ...prev, isOpen: false }))}>
                    <div className="text-center">
                        ...
                        <div className="flex gap-3 justify-center">
                            ...
                            <button onClick={confirmModal.onConfirm} className="px-6 py-2.5 bg-blue-600 text-white font-semibold rounded-xl shadow-lg">Ya, Salin Sekarang</button>
                        </div>
                    </div>
                </Modal>
            )}
