...
import {
    Save, FileSpreadsheet, FileText, Printer, Zap, Calendar, Loader2
} from 'lucide-react';
...
const PromesView: React.FC<PromesViewProps> = ({ grade, subject, semester, year, schedules, activeTab, userProfile, signingLocation, sharedEfektifData, subjects, schoolDays = 6 }) => {
    const { user } = useAuth();
    const [protaSource, setProtaSource] = useState<ProtaRow[]>([]);

    const getInitialTemplate = useCallback((): PekanEfektifItem[] => {
    ...
    const [pekanEfektifSource, setPekanEfektifSource] = useState<PekanEfektifItem[]>(getInitialTemplate());
    const [promesData, setPromesData] = useState<Record<string, string>>({});
    const [userHolidays, setUserHolidays] = useState<HolidayData[]>([]);
    const [loading, setLoading] = useState(false);
    const [programId, setProgramId] = useState<string | null>(null);
    const [isExporting, setIsExporting] = useState('');

    useEffect(() => {
        if (sharedEfektifData?.pekanEfektif && sharedEfektifData.pekanEfektif.length > 0) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                setUserHolidays(hSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as unknown as HolidayData)));
            } catch (error) {
        ...
        fetchData();
        return () => { ignore = true; };
    }, [grade, subject, semester, year, activeTab, getInitialTemplate, user]);

    const getHolidayForWeek = (monthName: string, wIndex: number) => {
        const monthNum = MONTH_MAP[monthName as keyof typeof MONTH_MAP];
    ...
    const formatHolidayRange = (h: HolidayData) => {
    ...
    const handleSave = async () => {
        ...
        protaSource.forEach(row => {
            ...
            pekanEfektifSource.forEach((month, mIndex) => {
                for (let w = 0; w < parseInt(String(month.totalWeeks || 4)); w++) distributed += parseInt(promesData[`${row.id}_${mIndex}_${w}`] || '0');
            });
            if (distributed !== parseInt(String(row.jp))) errors.push(`KD ${(row.kd || '').substring(0, 20)}... (Target: ${row.jp}, Isi: ${distributed})`);
        });
        if (errors.length > 0) {
        ...
        try {
            ...
            protaSource.forEach(row => {
                pekanEfektifSource.forEach((month, mIndex) => {
                    for (let w = 0; w < parseInt(String(month.totalWeeks || 4)); w++) {
            ...
            if (err.code === 'not-found' || err.message?.includes('No document to update')) {
                try {
                    await setDoc(doc(db, 'teachingPrograms', programId), {
                        userId: user.uid, subject, gradeLevel: grade, academicYear: year, semester,
    ...
    const handleExportWord = async () => {
        ...
        try {
            ...
            pekanEfektifSource.forEach(m => {
                ...
                for (let w = 1; w <= Number(m.totalWeeks || 4); w++) weekHeader += `<th style="${S.header} width: 25px;">${w}</th>`;
            });
            const rows = protaSource.map((row, i) => {
                let cells = '';
                pekanEfektifSource.forEach((m, mIndex) => {
                    for (let w = 0; w < (parseInt(String(m.totalWeeks || 4))); w++) {
                        const key = `${row.id}_${mIndex}_${w}`;
    ...
    const handleExportExcel = async () => {
        ...
        try {
            ...
            pekanEfektifSource.forEach(month => {
                const wc = parseInt(String(month.totalWeeks || 4));
                header1.push(month.name);
                for (let i = 1; i < wc; i++) header1.push('');
                for (let i = 1; i <= wc; i++) header2.push(`P${i}`);
            });
            const rows = protaSource.map((row, i) => {
                const r: (string | number)[] = [i + 1, `${row.kd}\n${row.materi}`, parseInt(String(row.jp || 0))];
                pekanEfektifSource.forEach((month, mIndex) => {
                    for (let w = 0; w < parseInt(String(month.totalWeeks || 4)); w++) {
            ...
            let col = 3;
            pekanEfektifSource.forEach(m => {
                const wc = parseInt(String(m.totalWeeks || 4));
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            const baseHeader = [
                ...
                { content: 'Tujuan Pembelajaran', rowSpan: 2, styles: { halign: 'left' as const, valign: 'middle' as const } },
                { content: 'Lingkup Materi', rowSpan: 2, styles: { halign: 'left' as const, valign: 'middle' as const } },
                { content: 'JP', rowSpan: 2, styles: { halign: 'center' as const, valign: 'middle' as const } }
            ];
            const monthHeaders: { content: string; colSpan: number; styles: { halign: 'center' } }[] = [], weekHeaders: { content: string; styles: { halign: 'center' } }[] = [];
            pekanEfektifSource.forEach(m => {
                monthHeaders.push({ content: m.name, colSpan: parseInt(String(m.totalWeeks || 4)), styles: { halign: 'center' } });
                for (let w = 1; w <= (parseInt(String(m.totalWeeks || 4))); w++) weekHeaders.push({ content: w.toString(), styles: { halign: 'center' } });
            });

            const body = protaSource.map((row, i) => {
                const r: (number | string | { content: string; styles: { halign: 'left' | 'center' | 'right'; fontSize?: number; fillColor?: [number, number, number]; textColor?: [number, number, number]; fontStyle?: 'bold' | 'italic' | 'normal' | 'bolditalic' } })[] = [i + 1, { content: `${row.elemen ? `[${row.elemen}]\n` : ''}${row.kd}`, styles: { halign: 'left', fontSize: 6.5 } }, row.materi, row.jp];
                pekanEfektifSource.forEach((m, mIndex) => {
                    for (let w = 0; w < (parseInt(String(m.totalWeeks || 4))); w++) {
                        const cellKey = `${row.id}_${mIndex}_${w}`;
                        const holiday = getHolidayForWeek(m.name, w);
                        const val = promesData[cellKey];
                        if (holiday?.isBlocking) r.push({ content: 'L', styles: { fillColor: [255, 235, 238] as [number, number, number], textColor: [198, 40, 40] as [number, number, number], halign: 'center' } });
                        else if (val && val !== '0') r.push({ content: val.toString(), styles: { fillColor: [232, 245, 233] as [number, number, number], fontStyle: 'bold', halign: 'center' } });
            ...
            autoTable(doc, {
                ...
                theme: 'grid', headStyles: { fillColor: [37, 99, 235] as [number, number, number], halign: 'center', lineWidth: 0.1 },
            ...
            let curY = (doc as unknown as { lastAutoTable: { finalY: number } }).lastAutoTable.finalY + 10;
            if (semesterHolidays.length > 0) {
    ...
    const updateCell = (protaId: string, monthIndex: number, weekIndex: number, value: string) => {
    ...
    const handleAutoDistribute = () => {
        ...
        let weeklyJP = 0;
        if (sharedEfektifData && sharedEfektifData.jpPerWeek && sharedEfektifData.jpPerWeek > 0) {
        ...
        let currM = 0, currW = 0;
        protaSource.forEach(row => {
            let remaining = parseInt(String(row.jp));
            while (remaining > 0) {
                while (currM < pekanEfektifSource.length) {
                    const month = pekanEfektifSource[currM];
                    const totalW = parseInt(String(month.totalWeeks || 4));
                    const nonEff = parseInt(String(month.nonEffectiveWeeks || 0));
                ...
                currW++;
                if (currW >= parseInt(String(pekanEfektifSource[currM].totalWeeks || 4))) { currW = 0; currM++; }
            }
        });
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, rIndex: number, mIndex: number, wIndex: number) => {
        ...
        if (e.key === 'ArrowUp') { e.preventDefault(); nR = Math.max(0, rIndex - 1); }
            ...
            else if (mIndex > 0) { nM = mIndex - 1; nW = parseInt(String(pekanEfektifSource[nM].totalWeeks || 4)) - 1; }
        } else if (e.key === 'ArrowRight') {
            e.preventDefault();
            const wc = parseInt(String(pekanEfektifSource[mIndex].totalWeeks || 4));
                    ...
                    <table className="w-full text-xs text-center border-collapse">
                        <thead className="bg-blue-600 text-white uppercase font-bold sticky top-0 z-30">
                            <tr>
                                ...
                                {pekanEfektifSource.map((month) => (
                                    <th key={month.name} colSpan={parseInt(String(month.totalWeeks || 4))} className="px-2 py-2 border border-blue-500 min-w-[120px]">{month.name}</th>
                                ))}
                            </tr>
                            <tr>
                                {pekanEfektifSource.map((month, mIndex) => {
                                    return Array.from({ length: parseInt(String(month.totalWeeks || 4)) }).map((_, wIndex) => (
                        ...
                        <tbody className="bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200">
                            {!loading && protaSource.map((row, index) => {
                                let currentSum = 0;
                                pekanEfektifSource.forEach((month, mIndex) => {
                                    for (let w = 0; w < parseInt(String(month.totalWeeks || 4)); w++) currentSum += parseInt(promesData[`${row.id}_${mIndex}_${w}`] || '0');
                                });
                                const isMatch = currentSum === parseInt(String(row.jp));
                                const isOver = currentSum > parseInt(String(row.jp));
                                return (
                                    <tr key={row.id}>
                                        ...
                                        {pekanEfektifSource.map((month, mIndex) => {
                                            return Array.from({ length: parseInt(String(month.totalWeeks || 4)) }).map((_, wIndex) => {
                                                ...
                                                const totalWeeks = parseInt(String(monthData.totalWeeks || 4));
                                                ...
                                                return (
                                                    <td key={cellKey} title={holiday ? holiday.name : isManualNonEffective ? 'Pekan Tidak Efektif' : ''} className={`border border-gray-200 dark:border-gray-700 p-0 hover:bg-gray-50 ${cellBg}`}>
                                                        {(!isBlockingHoliday && !isManualNonEffective) ? (
                                                            <input id={`promes-input-${index}-${mIndex}-${wIndex}`} type="text" value={val} onChange={(e) => updateCell(String(row.id), mIndex, wIndex, e.target.value)} onKeyDown={(e) => handleKeyDown(e, index, mIndex, wIndex)} className="w-full h-8 text-center bg-transparent focus:ring-1 focus:ring-blue-500 outline-none font-medium text-[11px]" />
                                                        ) : (
