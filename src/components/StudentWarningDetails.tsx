...
interface StudentWarningDetailsProps {
  student: StudentItem;
...
const StudentWarningDetails: React.FC<StudentWarningDetailsProps> = ({ student, activeSemester, academicYear }) => {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [details, setDetails] = useState<DetailsState>({ grades: [], attendance: [], infractions: [] });
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        ...
        const grades = gradesSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as GradeItem));
        ...
        const attendance = attendanceSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as AttendanceItem));
        ...
        const infractions = infractionsSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as InfractionItem));
    ...
    fetchDetails();
  }, [student, activeSemester, academicYear, user]);
                ...
                <li key={grade.id} className="p-2 bg-gray-50 dark:bg-gray-700 rounded-md">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">{formatDate(grade.date)}</p>
                    </div>
            ...
            <ul className="space-y-2">
              {details.infractions.map(infraction => (
                <li key={infraction.id} className="p-2 bg-red-50 dark:bg-red-900/20 rounded-md">
                  <p className="font-semibold">{infraction.infractionType} <span className="font-normal text-sm">({formatDate(infraction.date)})</span></p>
            ...
            <ul className="space-y-2">
              {details.attendance.map(att => (
                <li key={att.id} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700 rounded-md">
                  <span>{formatDate(att.date, 'id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
