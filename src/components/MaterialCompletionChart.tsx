...
const MaterialCompletionChart: React.FC = () => {
        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="bg-white/80 dark:bg-black/80 backdrop-blur-xl p-4 rounded-2xl shadow-2xl border border-white/20 dark:border-white/10 ring-1 ring-black/5">
                    <p className="font-black text-gray-900 dark:text-white mb-2 tracking-tight">{chartPayload.name}</p>
                    <div className="space-y-1.5 text-xs">
                        <div className="flex items-center justify-between gap-4">
                            <span className="text-gray-500 dark:text-gray-400 font-bold uppercase tracking-widest">Jumlah</span>
                            <span className="text-gray-900 dark:text-white font-black">{chartPayload.value} <span className="text-[10px] font-bold text-gray-400">Jurnal</span></span>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                            <span className="text-gray-500 dark:text-gray-400 font-bold uppercase tracking-widest">Persentase</span>
                            <span className="text-gray-900 dark:text-white font-black">{((chartPayload.value / (total || 1)) * 100).toFixed(1)}%</span>
                        </div>
                    </div>
                </div>
            );
