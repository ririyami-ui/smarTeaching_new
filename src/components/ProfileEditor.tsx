...
export default function ProfileEditor() {
  ...
  const [title, setTitle] = useState<string>('Bapak/Ibu'); // New State: Sapaan
  const [nip, setNip] = useState<string>('');
  const [school, setSchool] = useState<string>('');
  const [schoolLevel, setSchoolLevel] = useState<string>('SD'); // New State: Jenjang
  ...
  const [connectionStatus, setConnectionStatus] = useState<{ type: string; message: string }>({ type: '', message: '' });
  const [academicYear, setAcademicYear] = useState<string>('');
  const [activeSemester, setActiveSemester] = useState<string>('Ganjil');
  const [testingKey, setTestingKey] = useState<boolean>(false);
  const [geminiApiKey, setGeminiApiKey] = useState<string>('');
  const [showApiKey, setShowApiKey] = useState<boolean>(false);
  const [geminiModel, setGeminiModel] = useState<string>('gemini-3.1-flash-lite');
  const [academicWeight, setAcademicWeight] = useState<number>(50);
  const [attitudeWeight, setAttitudeWeight] = useState<number>(50);
      ...
      if (user) {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            setGeminiModel(profileData.geminiModel || 'gemini-3.1-flash-lite');
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    const profileSchema = z.object({
      ...
      principalNip: z.string().min(1),
      academicYear: z.string().min(1),
      activeSemester: z.enum(['Ganjil','Genap']),
      geminiModel: z.string().min(1),
      geminiApiKey: z.string().optional(),
      academicWeight: z.number().min(0).max(100),
      attitudeWeight: z.number().min(0).max(100),
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      } else if (errMsg.includes("API_KEY_INVALID") || errMsg.toLowerCase().includes("invalid")) {
        msg = "API Key tidak valid. Silakan periksa kembali.";
      } else {
        msg = "Gagal tes koneksi: " + errMsg;
      }
      ...
      <form onSubmit={handleUpdateProfile} className="space-y-6">
        ...
        <div className="space-y-4 pt-4">
          ...
          <div className="space-y-1 mt-4">
            ...
            >
              <option value="gemini-3.1-flash-lite">Gemini 3.1 Flash-Lite (Sangat Cepat & Efisien)</option>
