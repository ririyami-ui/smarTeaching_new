...
export default function ScheduleEditor({ scheduleData, onSave, onClose, subjects, classes }: ScheduleEditorProps) {
  ...
  useEffect(() => {
    if (scheduleData) {
      ...
      setStartPeriod(String(scheduleData.startPeriod ?? ''));
      setEndPeriod(String(scheduleData.endPeriod ?? ''));
  ...
  return (
    <form onSubmit={handleUpdateSchedule} className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {/* Type Toggle */}
      <div className="md:col-span-2 mb-2 p-1 bg-gray-100 dark:bg-gray-800 rounded-lg flex">
        <button
          type="button"
          onClick={() => setScheduleType('teaching')}
          className={`flex-1 py-1 px-3 rounded text-sm font-medium transition ${scheduleType === 'teaching' ? 'bg-blue-600 text-white shadow' : 'text-gray-600 dark:text-gray-400'}`}
        >
          Mengajar
        </button>
        <button
          ...
          className={`flex-1 py-1 px-3 rounded text-sm font-medium transition ${scheduleType === 'non-teaching' ? 'bg-pink-500 text-white shadow' : 'text-gray-600 dark:text-gray-400'}`}
        >
          Non-KBM
        </button>
      </div>
      ...
      <div className="mb-4">
        <label htmlFor="day" className="block text-gray-700 dark:text-gray-350 text-sm font-bold mb-2">Hari:</label>
        <StyledSelect
          id="day"
          value={day}
      ...
      {scheduleType === 'teaching' ? (
        <>
          <div className="mb-4">
            <label htmlFor="class" className="block text-gray-700 dark:text-gray-350 text-sm font-bold mb-2">Kelas:</label>
            <StyledSelect
              id="class"
              value={selectedClass}
          ...
          <div className="mb-4">
            <label htmlFor="startPeriod" className="block text-gray-700 dark:text-gray-350 text-sm font-bold mb-2">Jam ke:</label>
            <StyledInput
              type="number"
              id="startPeriod"
              value={startPeriod}
          ...
          <div className="mb-4">
            <label htmlFor="endPeriod" className="block text-gray-700 dark:text-gray-350 text-sm font-bold mb-2">Sampai jam ke:</label>
            <StyledInput
              type="number"
              id="endPeriod"
              value={endPeriod}
        ...
        <div className="mb-4 md:col-span-2">
          <label htmlFor="activityName" className="block text-gray-700 dark:text-gray-350 text-sm font-bold mb-2">Nama Kegiatan:</label>
          <StyledInput
            type="text"
            id="activityName"
            value={activityName}
      ...
      <div className="mb-4">
        <label htmlFor="startTime" className="block text-gray-700 dark:text-gray-350 text-sm font-bold mb-2">Waktu Mulai:</label>
        <StyledInput
          type="time"
          id="startTime"
          value={startTime}
      ...
      <div className="mb-4">
        <label htmlFor="endTime" className="block text-gray-700 dark:text-gray-350 text-sm font-bold mb-2">Waktu Selesai:</label>
        <StyledInput
          type="time"
          id="endTime"
          value={endTime}
      ...
      {scheduleType === 'non-teaching' && (
        <div className="mb-4 md:col-span-2">
          <label htmlFor="classOptional" className="block text-gray-700 dark:text-gray-350 text-sm font-bold mb-2">Kelas (Opsional):</label>
          <StyledSelect
            id="classOptional"
            value={selectedClass}
            onChange={(e) => setSelectedClass(e.target.value)}
      ...
      {scheduleType === 'teaching' && (
        <div className="mb-4 md:col-span-2">
          <label htmlFor="subject" className="block text-gray-700 dark:text-gray-350 text-sm font-bold mb-2">Mata Pelajaran:</label>
          <StyledSelect
            id="subject"
