...
interface PekanEfektifMonth {
  name: string;
  totalWeeks: string | number;
  nonEffectiveWeeks: string | number;
}
...
const ClockDisplay: React.FC<ClockDisplayProps> = ({ size = 'lg', variant = 'card', showProgress = false, activeSchedule = null }) => {
  ...
  const [pekanEfektifData, setPekanEfektifData] = useState<PekanEfektifMonth[]>([]);
  const [userHolidays, setUserHolidays] = useState<UserHoliday[]>([]);
  const [academicYear, setAcademicYear] = useState('');
  const [, setActiveSemester] = useState('');

  const timerRef = useRef<NodeJS.Timeout | null>(null);

  useEffect(() => {
    const fetchSchoolNameAndSettings = async (user: User) => {
      if (user) {
        try {
          ...
          if (docSnap.exists()) {
            ...
            if (showProgress && data.activeSemester && data.academicYear) {
              ...
              setUserHolidays(hSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as UserHoliday)));
              ...
              if (calSnap.exists() && calSnap.data().pekanEfektif) {
                ...
                const total = pepData.reduce((acc: number, curr: PekanEfektifMonth) =>
  ...
  const getHolidayForWeek = useCallback((monthName: string, wIndex: number) => {
  ...
  useEffect(() => {
    const calculateProgress = () => {
      if (pekanEfektifData.length > 0 && academicYear) {
        ...
        const manualTargetTotal = pekanEfektifData.reduce((acc, curr) =>
          acc + Math.max(0, (parseInt(String(curr.totalWeeks || 0)) - parseInt(String(curr.nonEffectiveWeeks || 0)))), 0);
        ...
        pekanEfektifData.forEach(month => {
          const totalWeeksInMonth = parseInt(String(month.totalWeeks || 4));
          const manualEffectiveInMonth = Math.max(0, totalWeeksInMonth - parseInt(String(month.nonEffectiveWeeks || 0)));
    ...
    calculateProgress();
  }, [pekanEfektifData, userHolidays, currentTime, academicYear, effectiveWeeks, getHolidayForWeek]);
  ...
  const formatIndonesianDate = (m: moment.Moment) => {
  ...
  const SegmentDisplay = ({ value, label }: { value: string, label: string }) => (
