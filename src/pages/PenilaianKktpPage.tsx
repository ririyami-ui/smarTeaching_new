...
const PenilaianKktpPage: React.FC = () => {
    const { user } = useAuth();
    ...
    const [manualCriteria, setManualCriteria] = useState<ManualCriterion[]>([]);
    const [isManualMode, setIsManualMode] = useState(false);
    const [assessmentScores, setAssessmentScores] = useState<Record<string, Record<number, number>>>({});
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const rppData = rppSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as unknown as RPPItem));
                ...
                const classData = classSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as ClassItem));
                ...
                setSubjects(subjectSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as SubjectItem)));
    ...
    const fetchStudents = useCallback(async () => {
        ...
        try {
            ...
            snapId.docs.forEach(doc => studentMap.set(doc.id, { id: doc.id, ...doc.data() } as StudentItem));
            snapRombel.docs.forEach(doc => {
                if (!studentMap.has(doc.id)) studentMap.set(doc.id, { id: doc.id, ...doc.data() } as StudentItem);
            });
    ...
    useEffect(() => {
        const fetchExistingAssessment = async () => {
            if (!selectedRpp || !selectedClass || !user) return;
    ...
    }, [selectedRpp, selectedClass, activeSemester, academicYear, user]);
    ...
    const handleExtractWithAI = async () => {
        ...
        try {
            const aiKktpData = await extractKKTPFromRPP(rpp.content, String(userProfile?.preferredModel || ''));
            const kktpCriteria = (aiKktpData as { criteria?: unknown[] }).criteria;
            if (kktpCriteria && kktpCriteria.length > 0) {
                setKktpData(aiKktpData as unknown as KKTPData);
                setIsManualMode(false);
                setManualCriteria([]);
    ...
    // Scoring Logic
    const handleScoreChange = (studentId: string, aspectIndex: number, score: number) => {
    ...
    const removeManualCriterion = (index: number) => {
    ...
    const updateManualCriterion = (index: number, value: string) => {
    ...
    const handleSave = async (syncToGrades = false, confirmedType: string | null = null) => {
        if (!selectedRpp || !selectedClass || !user) {
        ...
        try {
            ...
            if (!rpp || !classData) {
                toast.error("Data RPP atau Kelas tidak valid");
                setIsSaving(false);
                return;
            }
            ...
            const assessmentDocData = {
                ...
                kktpType: isManualMode ? 'Manual Rubrik' : kktpData?.type || 'Unknown',
                ...
                students.forEach(student => {
                    ...
                    if (finalScore > 0) {
                        ...
                        batch.set(gradeRef, {
                            ...
                            date: assessmentDate,
                            material: rpp.topic,
                            assessmentType: confirmedType,
                            score: finalScore,
                            semester: activeSemester,
                            academicYear: academicYear,
                            timestamp: serverTimestamp(),
                            source: 'kktp-digital',
                            kktpAssessmentId: newAssessmentDocRef.id
                        }, { merge: true });
                    }
                });
                await batch.commit();
                    ...
                    >
                        ...
                        {classes
                            .filter(c => {
                                ...
                                const level = String(c.level || c.grade || '').toLowerCase();
                                const filter = String(gradeFilter).toLowerCase();
                                return level === filter || level.includes(filter);
                            })
                            .map(cls => (
                                        ...
                                        <tr key={student.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30 transition-colors">
                                            ...
                                            {(isManualMode ? manualCriteria : (kktpData?.criteria || [])).map((c, i) => (
                                                <td key={i} className="px-4 py-4 text-center">
                                                        ...
                                                        <div className="flex items-center justify-center gap-1">
                                                            {[1, 2, 3, 4].map(level => (
                                                                <button
                                                                    ...
                                                                    title={(!isManualMode && (c as KKTPCriterion).levels) ? (c as KKTPCriterion).levels![level - 1]?.desc : `Level ${level}`}
                                                                >
                                                                    {level}
                                                                </button>
                                                            ))}
                                                    ...
                                                    {!isManualMode && kktpData && kktpData.type === 'Deskripsi Kriteria' && (
                                                        <button
                                                    ...
                                                    {!isManualMode && kktpData && kktpData.type === 'Interval Nilai' && (
                                                        <input
                                                            type="number"
                                                            min="0"
                                                            max="100"
