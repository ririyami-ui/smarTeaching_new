...
import { Trash2, Settings2, Plus, Edit2, Save, X, ShieldCheck, Star, Sparkles, UserCheck } from 'lucide-react';
...
const PelanggaranPage = () => {
  ...
  const [activeTab, setActiveTab] = useState('infractions'); // 'infractions' or 'appreciations'
  ...
  const [dynamicAppreciationTypes, setDynamicAppreciationTypes] = useState([]);
  ...
  const [settingsTab, setSettingsTab] = useState('infractions'); // 'infractions' or 'appreciations'
  ...
  const DEFAULT_APPRECIATION_TYPES = {
  ...
  const fetchDynamicTypes = async () => {
    ...
    try {
      ...
      const iq = query(collection(db, 'infraction_types'), where('userId', '==', auth.currentUser.uid));
      const iSnapshot = await getDocs(iq);
      const iTypes = iSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      iTypes.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
      setDynamicInfractionTypes(iTypes);

      // Fetch Appreciation Types
      const aq = query(collection(db, 'appreciation_types'), where('userId', '==', auth.currentUser.uid));
      const aSnapshot = await getDocs(aq);
      const aTypes = aSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      aTypes.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
      setDynamicAppreciationTypes(aTypes);

      // Auto-migration for infractions if empty
      if (iTypes.length === 0) {
        const batch = writeBatch(db);
        Object.entries(DEFAULT_INFRACTION_TYPES).forEach(([name, data]) => {
          batch.set(doc(collection(db, 'infraction_types')), { ...data, name, userId: auth.currentUser.uid });
        });
        await batch.commit();
        fetchDynamicTypes();
      }

      // Auto-migration for appreciations if empty
      if (aTypes.length === 0) {
        const batch = writeBatch(db);
        Object.entries(DEFAULT_APPRECIATION_TYPES).forEach(([name, data]) => {
          batch.set(doc(collection(db, 'appreciation_types')), { ...data, name, userId: auth.currentUser.uid });
        });
        await batch.commit();
        fetchDynamicTypes();
      }
  ...
  const handleSaveAppreciation = async () => {
    if (!selectedStudent || !appreciationType) {
  ...
  const calculateScore = (infractions, appreciations) => {
    const totalDeducted = infractions.reduce((acc, curr) => acc + (curr.points || 0), 0);
    const totalStars = appreciations.reduce((acc, curr) => acc + (curr.points || 0), 0);
  ...
  const handleCheckRoleModels = async () => {
    ...
    try {
      ...
      // Fetch all infractions for this class in this semester
      const iQuery = query(
      ...
      // Candidates are students with NO infractions
      const candidates = students.filter(s => !studentWithInfractions.has(s.id));

      // Also check if they ALREADY received Bintang Konsistensi today or this week?
      // For now, just show the list of clean students.
  ...
  const handleAwardConsistencyStar = async (studentId) => {
    try {
      const classObj = classes.find(c => c.rombel === selectedClass || c.id === selectedClass);
                        ...
                        <tr key={app.id} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors">
                          <td className="py-4 px-6">
                            <div className="flex flex-col">
                              ...
                              <span className="text-[10px] text-gray-400">
                                {new Date(app.date).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} WIB
                              </span>
                            </div>
                          </td>
                          <td className="py-4 px-6">
                          ...
                          <td className="py-4 px-6 text-center">
                            <button
                              ...
                              className="p-2 text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors rounded-lg hover:bg-bg-red-50 dark:hover:bg-red-900/20"
                            >
                              <Trash2 size={16} />
                            </button>
                          </td>
