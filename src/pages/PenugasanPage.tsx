...
import { ClipboardCheck, Trash, Plus, Calendar, CheckCircle2, Circle, AlertCircle, Edit3, X, Trash2 } from 'lucide-react';
import { collection, getDocs, query, where, doc, serverTimestamp, orderBy, deleteDoc, addDoc, updateDoc } from 'firebase/firestore';
...
const PenugasanPage: React.FC = () => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const tasksRef = collection(db, 'studentTasks');
      const q = query(
        tasksRef,
      ...
      setTasks(querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as unknown as TaskItem)));
    } catch (error) {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const classesQuery = query(classesRef, where('userId', '==', user.uid));
        const subjectsQuery = query(subjectsRef, where('userId', '==', user.uid));
        ...
        setSubjects(subjectsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as SubjectItem)));
      } catch (error) {
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedClass || !selectedSubject || !taskTitle || !deadline || !user) {
    ...
    try {
      ...
      const taskData: Omit<TaskItem, 'id' | 'status'> = {
        userId: user.uid,
