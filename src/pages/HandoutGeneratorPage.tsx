...
interface BSKAPSemesterData {
  materi_inti?: BSKAPMateriItem[];
  elemen?: string[];
  cp_snippet?: string;
}

interface BSKAPSubjectData {
  ganjil?: BSKAPSemesterData;
  genap?: BSKAPSemesterData;
}

interface BSKAPData {
  subjects?: Record<string, Record<string, Record<string, BSKAPSubjectData>>>;
}

const BSKAP_DATA = BSKAP_DATA_RAW as BSKAPData;
...
interface LocalMermaidProps {
  content: string;
}

const HandoutGeneratorPage: React.FC = () => {
    const { user } = useAuth();
    ...
    const LocalMermaid: React.FC<LocalMermaidProps> = ({ content }) => {
    ...
    const MermaidRenderer: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [selectedRPPId, setSelectedRPPId] = useState<string>(''); // To track selected RPP

    // Generation States
    const [sourceType, setSourceType] = useState<string>('rpp'); // 'rpp', 'atp', or 'manual'
    interface AtpItem {
      materi: string;
      elemen: string;
      tp?: string;
    ...
    const [selectedAtpItem, setSelectedAtpItem] = useState<AtpItem | null>(null);
    const aiTask = tasks.handout;
    const isGenerating = aiTask.status === 'loading';
    const [generatedContent, setGeneratedContent] = useState<string>(typeof aiTask.result === 'string' ? aiTask.result : '');
    const [generationProgress, setGenerationProgress] = useState<{ stage: string; message: string; percentage: number }>({ stage: '', message: aiTask.progress || '', percentage: aiTask.status === 'success' ? 100 : 0 });

    // History & Saving States
    const [savedHandouts, setSavedHandouts] = useState<HandoutItem[]>([]);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const handouts = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as HandoutItem));
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            try {
                ...
                const uniqueLevels = [...new Set(classesSnap.docs.map(doc => doc.data().level))].sort() as string[];
                ...
                const rpps = rppSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as RPPItem));
                ...
                if (aiTask.params?.topic) {
                    setTopic(String(aiTask.params.topic));
                }
                if (aiTask.params?.subject) {
                    const subMatch = fetchedSubjects.find(s => s.name === String(aiTask.params!.subject));
    ...
    const handleRPPSelect = (rppId: string) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            await startGeneration('handout', async (context: { onProgress: (msg: string) => void }) => {
                ...
                const atpElemen = selectedAtpItem?.elemen;
                return await generateHandout({
                    subject: subjectName,
                    gradeLevel: selectedGrade,
                    materi: sourceType === 'atp' ? atpMateri : topic,
                    kd: sourceType === 'atp' ? atpKd : selectedRPP?.kd,
                    elemen: sourceType === 'atp' ? atpElemen : selectedRPP?.elemen,
                ...
                topic: sourceType === 'atp' ? (selectedAtpItem?.materi || topic) : topic,
                subject: subjectName
            });
        } catch {
            // Handled by context
    ...
    useEffect(() => {
        if (aiTask.status === 'success' && aiTask.result) {
            setGeneratedContent(typeof aiTask.result === 'string' ? aiTask.result : '');
        }
    ...
    const handleDownloadDocx = async () => {
        ...
        try {
            const previewContainer = document.getElementById('handout-preview');
            if (!previewContainer) throw new Error("Preview container not found");

            // Clone the preview element to modify it for export
            const previewEl = previewContainer.cloneNode(true) as HTMLElement;
            const originalSvgs = previewContainer.querySelectorAll('svg');
            ...
            for (let i = 0; i < clonedSvgs.length; i++) {
                ...
                const originalSvg = originalSvgs[i] as SVGSVGElement;
            ...
            toast.error("Gagal mengunduh dokumen: " + errorMessage, { id: 'word-export' });
        }
    };
    ...
    const handleLoad = (item: HandoutItem) => {
        ...
        <div className="flex h-screen bg-bg-light dark:bg-bg-dark font-poppins">
                    ...
                    <div className="max-w-7xl mx-auto space-y-6">
                        ...
                        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 card-glass text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition shadow-sm"
                            >
                                <History size={18} />
                                Riwayat
                            </button>
                                ...
                                {sourceType === 'rpp' && (
                                    <div className="animate-fade-in-down">
                                        ...
                                        <div className="relative">
                                            ...
                                            <div className="absolute right-3 top-3 pointer-events-none border-l-4 border-r-4 border-t-4 border-t-gray-400 border-l-transparent border-r-transparent w-0 h-0"></div>
                                        </div>
                                        ...
                                        <div className="relative">
                                            ...
                                            >
                                                ...
                                                {(() => {
                                                    ...
                                                    const ganjilMateri = subjectData.ganjil?.materi_inti?.map((m: BSKAPMateriItem) => ({
                                                        materi: m.materi || '',
                                                        elemen: m.elemen || subjectData.ganjil?.elemen?.join(', ') || 'Umum',
                                                        tp: subjectData.ganjil?.cp_snippet
                                                    })) || [];

                                                    const genapMateri = subjectData.genap?.materi_inti?.map((m: BSKAPMateriItem) => ({
                                                        materi: m.materi || '',
            ...
            {showHistory && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="card-glass rounded-2xl shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
