...
if (attendance.length > 0) {
      ...
      const tpMap: Record<string, { total: number; passed: number; failed: number; failedStudents: { name: string; id: string }[] }> = {};
      grades.forEach(g => {
        ...
        if (score >= 78) {
          ...
          tpMap[mat].failed++;
          tpMap[mat].failedStudents.push({ name: g.studentName as string, id: g.studentId as string });
        }
      });
      ...
      const tpMasteryData = Object.entries(tpMap).map(([material, d]) => ({
        ...
        failedStudents: d.failedStudents.map(s => s.name),
        failedStudentIds: d.failedStudents.map(s => s.id)
  ...
  return (
    <div className="p-3 sm:p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      {activeMode === 'ai' && report && !loading && analysisData && (
      ...
      {activeMode === 'tp' && analysisData && !loading && (
        <div className="space-y-6 animate-fade-in-up">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="card-glass p-4 border border-blue-100 bg-blue-50/30">
              <p className="text-xs font-bold text-blue-600 uppercase mb-1">Total Materi (TP)</p>
              <p className="text-2xl font-black text-blue-900">{tpMastery.length}</p>
            </div>
            <div className="card-glass p-4 border border-green-100 bg-green-50/30">
              <p className="text-xs font-bold text-green-600 uppercase mb-1">TP Tuntas Terendah</p>
              <p className="text-2xl font-black text-green-900">{tpMastery.length > 0 ? `${tpMastery[0].percentage}%` : '-'}</p>
            </div>
            <div className="card-glass p-4 border-amber-100 bg-amber-50/30">
              <p className="text-xs font-bold text-amber-600 uppercase mb-1">Ambang Batas (KKTP)</p>
              <p className="text-2xl font-black text-amber-900">78</p>
            </div>
          </div>
          <div className="card-glass overflow-hidden border border-gray-100">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-100/50 dark:bg-gray-700">
                  <th className="p-4 text-xs font-black uppercase text-gray-600 dark:text-gray-300">Materi / Tujuan Pembelajaran (TP)</th>
                  <th className="p-4 text-xs font-black uppercase text-gray-600 dark:text-gray-300 text-center">Peserta</th>
                  <th className="p-4 text-xs font-black uppercase text-gray-600 dark:text-gray-300 text-center">Tuntas</th>
                  <th className="p-4 text-xs font-black uppercase text-gray-600 dark:text-gray-300 text-center">Belum</th>
                  <th className="p-4 text-xs font-black uppercase text-gray-600 dark:text-gray-300">Progres Ketuntasan</th>
                </tr>
              </thead>
              <tbody>
                {tpMastery.map((tp, idx) => {
                  return (
                    <React.Fragment key={idx}>
                      <tr className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        <td className="p-4">
                          <p className="text-sm font-bold text-gray-800 dark:text-white">{tp.material}</p>
                        </td>
                        <td className="p-4 text-center font-bold text-gray-600">{tp.total}</td>
                        <td className="p-4 text-center font-bold text-green-600">{tp.passed}</td>
                        <td className="p-4 text-center font-bold text-red-600">{tp.failed}</td>
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                              <div 
                                className={`h-full transition-all duration-1000 ${tp.percentage >= 75 ? 'bg-green-500' : tp.percentage >= 50 ? 'bg-amber-500' : 'bg-red-500'}`}
                                style={{ width: `${tp.percentage}%` }}
                              />
                            </div>
                            <span className="text-xs font-black text-gray-700 dark:text-gray-300">{tp.percentage}%</span>
                          </div>
                        </td>
                      </tr>
                      {tp.failedStudents.length > 0 && (
                        <tr>
                          <td colSpan={5} className="p-4 pt-0">
                            <div className="p-3 bg-red-50/50 dark:bg-red-900/10 rounded-xl border border-red-100/50 dark:border-red-900/20">
                              <p className="text-[10px] font-black text-red-600 uppercase mb-2 flex items-center gap-2">
                                <ShieldAlert size={12} /> Butuh Remedial ({tp.failedStudents.length} Siswa):
                              </p>
                              <div className="flex flex-wrap gap-2">
                                {tp.failedStudents.map((name, sidx) => {
                                  const studentId = tp.failedStudentIds[sidx];
                                  return studentId ? (
                                    <button
                                      key={sidx}
                                      ...
                                      className="px-2 py-0.5 bg-white dark:bg-gray-800 text-[10px] font-bold text-blue-600 dark:text-blue-400 rounded-md border border-blue-200 dark:border-blue-700 shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all cursor-pointer"
                                    >
                                      {name}
                                    </button>
                                  ) : (
                                    <span key={sidx} className="px-2 py-0.5 bg-white dark:bg-gray-800 text-[10px] font-bold text-gray-600 dark:text-gray-400 rounded-md border border-gray-200 dark:border-gray-700 shadow-sm">
                                      {name}
                                    </span>
                                  );
                                })}
                ...
                {tpMastery.length === 0 && (
                  <tr>
                    <td colSpan={5} className="p-10 text-center text-gray-400 font-bold italic">
                      Belum ada data nilai yang diinput untuk kelas ini.
                    </td>
                  </tr>
                )}
              </tbody>
