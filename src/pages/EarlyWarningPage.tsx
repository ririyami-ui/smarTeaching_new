...
interface FlaggedStudent {
  ...
  rombel: string;
  warnings: string[];
  subjectsWithWarnings?: SubjectWarningInfo[];
...
const EarlyWarningPage: React.FC = () => {
  ...
  const [, setAllStudents] = useState<Record<string, unknown>[]>([]);
    ...
    const performAnalysis = async () => {
      if (currentUser) {
        ...
        try {
          ...
          setFlaggedStudents(flaggedResults as FlaggedStudent[]);
          setAllStudents(allStudentsData as Record<string, unknown>[]);
          setClasses(classesData.docs.map(doc => ({ id: doc.id, ...doc.data() } as ClassItem)).sort((a, b) => (a.rombel || '').localeCompare(b.rombel || '')));
          setSubjects(subjectsData.docs.map(doc => ({ id: doc.id, ...doc.data() } as SubjectItem)).sort((a, b) => (a.name || '').localeCompare(b.name || '')));
        } catch (err) {
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="h-16 w-16 animate-spin rounded-full border-4 border-t-primary border-r-transparent"></div>
