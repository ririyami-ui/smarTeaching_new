...
const DatabaseCleanupPage: React.FC = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        setLoading(true);
        try {
            const reportData = await generateCleanupReport(user.uid) as ReportData;
            setReport(reportData);
            toast.success('Laporan berhasil dibuat!');
        } catch (error: unknown) {
            console.error('Error generating report:', error);
            toast.error('Gagal membuat laporan: ' + (error instanceof Error ? error.message : 'Unknown error'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCleanup = async () => {
        ...
        setCleaning(true);
        try {
            const results = await executeAutoCleanup(user.uid, cleanupOptions);
            ...
            toast.error('Gagal membersihkan database: ' + (error instanceof Error ? error.message : 'Unknown error'));
        } finally {
            setCleaning(false);
        }
    };
