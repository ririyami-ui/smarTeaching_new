...
const AsistenGuruPage = () => {
  ...
  const fetchLiveContext = async (user) => {
    ...
    try {
      ...
      const [attSnap, gradesSnap, journalsSnap, infractionsSnap, starsSnap, holidaysSnap, studentsSnap] = await Promise.all([
        ...
        getDocs(query(collection(db, 'holidays'), where('userId', '==', userId))),
      ...
      const studentNames = Object.fromEntries(studentsSnap.docs.map(doc => [doc.id, doc.data().name]));
  ...
  const handleSendMessage = async () => {
    ...
    const userMessage = { role: 'user', parts: [{ text: input }], image: selectedImage };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-144px)] md:h-[calc(100vh-104px)] bg-gray-50 dark:bg-gray-900 overflow-hidden">
      ...
      <ChatInput
        input={input}
        onInputChange={(e) => { setInput(e.target.value); autoResizeTextarea(); }}
