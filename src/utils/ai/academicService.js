...
export const generateLessonPlan = async (data) => {
    try {
        ...
        const model = getModel(data.modelName, false, STRICT_DOCUMENT_BRAIN);
 ...
 */
export async function extractKKTPFromRPP(rppContent, modelName) {
    try {
        const prompt = getExtractKKTPPrompt(rppContent);
        const model = getModel(modelName, true, STRICT_DOCUMENT_BRAIN); // Use true for json response if necessary, but we'll try extractJSON first
        const result = await retryWithBackoff(() => model.generateContent(prompt));
